...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ClickAttrs.textfield(), ["test1", "test2"], os.path.pathsep.join(["test1", "test2"])),
        (ClickAttrs.filefield(), "test", "test"),
        (ClickAttrs.filefield(), ["test1", "test2"], os.path.pathsep.join(["test1", "test2"])),
        (ClickAttrs.filepathfield(), "test", "test"),
        (ClickAttrs.filepathfield(), ["test1", "test2"], os.path.pathsep.join(["test1", "test2"])),
...
@pytest.mark.parametrize(
    ("click_attrs", "envvar_values", "expected"),
    [
        (ClickAttrs.multi_value_widget(nargs=3, type=click.types.Path()), ["a", "b"], "Takes 3 values but 2 were given."),
        (ClickAttrs.multi_value_widget(nargs=2, type=click.types.File()), ["a", "b", "c"], "Takes 2 values but 3 were given."),
        (ClickAttrs.tuple_widget(types=(click.types.Path(), click.types.Path())), ["a", "b", "c"], "Takes 2 values but 3 were given."),
        (ClickAttrs.tuple_widget(types=(click.types.Path(), click.types.File())), ["a"], "Takes 2 values but 1 was given."),
        (ClickAttrs.tuple_widget(types=(click.types.File(), click.types.Path())), ["a", "c", "b"], "Takes 2 values but 3 were given."),
...
def test_set_envvar_fail(click_attrs:dict, envvar_values:str|list[str], expected:str|list[str]):
    ...
    with pytest.raises(click.exceptions.BadParameter) as exc_info:
        clickqt.qtgui_from_click(cli, True, " ").widget_registry[cli.name][param.name]
