...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ClickAttrs.intrange(min=0), -2.12, "'-2.12' is not a valid integer range."),
        (ClickAttrs.intrange(max=1), 2, "2 is not in the range x<=1."),
        (ClickAttrs.floatrange(min=0), -1.2, "-1.2 is not in the range x>=0."),
        (ClickAttrs.floatrange(min=0), "y", "'y' is not a valid float range."),
        (ClickAttrs.floatrange(max=1), 2.2, "2.2 is not in the range x<=1."),
