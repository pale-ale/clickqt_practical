...
from PySide6.QtWidgets import QLineEdit, QFileDialog, QApplication, QPushButton, QMessageBox, QWidget
from PySide6.QtCore import QTimer, SIGNAL, Signal
...
@pytest.mark.parametrize(
    ...
    [
        ...
        #(ClickAttrs.filepathfield(type_dict={"exists":False, "dir_okay":False}), "tests", "tests"), # Test does it not correctly, manually it works
...
def test_pathfield(monkeypatch:MonkeyPatch, click_attrs:dict, value:str, expected:str):
    ...
    def selectFile():
        class MessageBoxClosed(QWidget):
            messageBoxClosed = Signal(None)

        file_dialog:QFileDialog = QApplication.activeModalWidget()
        m = MessageBoxClosed()

        # Wait, until we have the QFileDialog object
        while file_dialog is None or not isinstance(file_dialog, QFileDialog): # See also https://github.com/pytest-dev/pytest-qt/issues/256
        ...
        def closeMessagebox():
            ...
            while messagebox is not None and not isinstance(messagebox, QFileDialog|QMessageBox):
                QApplication.processEvents()
