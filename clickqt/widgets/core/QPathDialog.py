    ...
    def eventFilter(self, watched:QObject, event:QEvent) -> bool:
        ...
        if btn:
            if event.type() == QEvent.Type.EnabledChange and not btn.isEnabled():
                ...
                self.base_dir = self.directory().absolutePath() # Save current dir, after changing fileMode it could be changed 
                if not self.exist:
    ...
    def openClicked(self):
        ...
        for model_index in self.tree_view.selectionModel().selectedIndexes():
            if model_index.column() == 0:
                self.selected_path = self.base_dir + QDir.separator() + str(model_index.data())
                self.selected_path.replace("/", QDir.separator())
                break
        
        if not self.selected_path and not self.exist: 
            self.selected_path = self.base_dir + QDir.separator() + self.findChild(QLineEdit, "fileNameEdit").text()
            self.selected_path.replace("/", QDir.separator())
            QDialog.accept(self)
        elif self.selected_path: # Don't close if the user selected an invalid file/folder
            QDialog.accept(self)